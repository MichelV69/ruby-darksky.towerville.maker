...
Then('Primary Economic Rating should be {int}') do |expect_value|
  expect(@subject.get_primary_economic_rating).to eq(expect_value)
end

Then('Primary Economic Rating should be valid') do
  expect(@subject.get_primary_economic_rating.round.to_i).to be_greater_than(0)
end
...
When('I set the Floor Count to {int}') do |new_FloorCount|
  ...
  puts "Number of Homes:  #{@subject.get_number_of_homes_estimate}"
end
...
Then('the Primary Economic Rating description should be {string}') do |expect_text|
  expect(@subject.get_primary_economic_rating_as_text).to eq(expect_text)
end
