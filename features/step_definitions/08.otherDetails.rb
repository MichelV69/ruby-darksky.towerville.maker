...
When('I use random_variance_by_primary_economic_rating') do
  @subject.shop_count_variance_percent = Towerville2056.random_variance_by_primary_economic_rating(@subject.primary_economic_rating)
end
...
Then('the number of shops should be around {int}') do |expectedShopCountEstimate|
  ...
  expect(@subject.shop_count_estimate).to be_greater_than(lowShopCount)
  expect(@subject.shop_count_estimate).to be_less_than(highShopCount)
end
...
Then('the Social Spaces data should be reasonable') do
  ...
  expect(@subject.social_spaces_data[:total_msq].to_i).to be_greater_than((@social_spaces_median_data[:total_msq] * low_variance).to_i)

  expect(@subject.social_spaces_data[:total_msq].to_i).to be_less_than((@social_spaces_median_data[:total_msq] * high_variance).to_i)

  expect(@subject.social_spaces_data[:size_ea_msq].to_i).to be_greater_than((@social_spaces_median_data[:size_ea_msq] * low_variance).to_i)

  expect(@subject.social_spaces_data[:size_ea_msq].to_i).to be_less_than((@social_spaces_median_data[:size_ea_msq] * high_variance).to_i)
end
...
When('I use green_spaces_data with set random_variance_by_primary_economic_rating fully randomized') do
  @subject.green_spaces_variance_percent = 0.0
  @green_spaces_median_data = @subject.social_spaces_data
  @subject.green_spaces_variance_percent = Towerville2056.random_variance_by_primary_economic_rating(@subject.primary_economic_rating)
end
...
Then('the Green Spaces data should be reasonable') do
  ...
  expect(@subject.green_spaces_data[:total_msq].to_i).to be_greater_than((@green_spaces_median_data[:total_msq] * low_variance).to_i)

  expect(@subject.green_spaces_data[:total_msq].to_i).to be_less_than((@green_spaces_median_data[:total_msq] * high_variance).to_i)

  expect(@subject.green_spaces_data[:size_ea_msq].to_i).to be_greater_than((@green_spaces_median_data[:size_ea_msq] * low_variance).to_i)

  expect(@subject.green_spaces_data[:size_ea_msq].to_i).to be_less_than((@green_spaces_median_data[:size_ea_msq] * high_variance).to_i)
end
